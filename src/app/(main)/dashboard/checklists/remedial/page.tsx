...
const Button = ({ children, onClick, type = "button", variant, size, disabled, className }: any) => (
  <button onClick={onClick} type={type} disabled={disabled} className={`px-4 py-2 rounded-lg font-semibold shadow-md transition-colors ${variant === 'outline' ? 'bg-white border border-gray-300 text-gray-700 hover:bg-gray-100' : variant === 'secondary' ? 'bg-gray-100 text-gray-700 hover:bg-gray-200' : 'bg-black text-white hover:bg-gray-800'} ${className || ''}`}>
    {children}
  </button>
);
const Input = ({ value, onChange, type = "text", placeholder, className = '' }: any) => (
  <input value={value} onChange={onChange} type={type} placeholder={placeholder} className={`w-full p-2 border border-gray-300 rounded-md focus:ring-gray-500 focus:border-gray-500 ${className}`} />
);
const Textarea = ({ value, onChange, placeholder, className = '' }: any) => (
  <textarea value={value} onChange={onChange} placeholder={placeholder} className={`w-full p-2 border border-gray-300 rounded-md focus:ring-gray-500 focus:border-gray-500 ${className}`} />
);
const Card = ({ children, ref }: any) => <div ref={ref} className="bg-white rounded-xl shadow-2xl overflow-hidden" data-slot="card">{children}</div>;
...
const RemedialActionChecklistPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        const response = await fetch(`http://127.0.0.1:5000/checklists/save`, {
            method: "POST",
  ...
  const downloadAsPdf = async () => {
    ...
    try {
        ...
        const response = await fetch(`http://127.0.0.1:5000/checklists/download-pdf`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(formData),
        });
      ...
      <Card ref={formRef}> 
        <CardHeader className="text-center bg-gray-50 border-b">
            <div className="flex items-center justify-center space-x-3">
                ...
                <div className="h-10 w-10 bg-gray-200 rounded-full flex items-center justify-center text-gray-800">
            ...
            <p className="text-xl font-semibold mt-1">REMEDIAL WORK CHECKLIST</p>
        </CardHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div className="space-y-2">
                <p className="font-semibold">CUSTOMER NAME: <span className="font-normal text-gray-700">{customerName}</span></p>
                <p className="font-semibold">CUSTOMER ADDRESS: <span className="font-normal text-gray-700">{customerAddress}</span></p>
                <p className="font-semibold">CUSTOMER TEL NO.: <span className="font-normal text-gray-700">{customerPhone}</span></p>
              </div>
              <div className="space-y-2 pt-2 md:pt-0">
                <div className="flex items-center space-x-2">
                  <label htmlFor="date" className="font-semibold w-24 flex-shrink-0">DATE:</label>
                  <Input 
                    id="date" 
                    type="date" 
                    value={date} 
                ...
                <div className="flex items-center space-x-2">
                  <label htmlFor="fitters" className="font-semibold w-24 flex-shrink-0">FITTERS:</label>
                  <Input 
                    id="fitters" 
                    value={fitters} 
            ...
            <h3 className="text-lg font-bold border-b pb-2 mt-8">Items Required for Remedial Action</h3>
              ...
              <table className="min-w-full divide-y divide-gray-200 border">
                <thead>
                  <tr className="bg-gray-50 text-xs font-medium text-gray-500 uppercase tracking-wider text-center">
                    ...
                    <th className="p-3 w-20">COLOUR</th>
                    <th className="p-3 w-20">SIZE</th>
                    <th className="p-3 w-16">QTY</th>
                    <th className="w-16 p-3 no-print">ACTION</th> 
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {checklistItems.map((item, index) => (
                    <tr key={item.id}>
                      <td className="p-3 text-center text-sm font-medium bg-gray-50">{index + 1}</td>
                      <td className="p-1">
                        <Textarea
                          value={item.item}
                          onChange={(e: any) => handleItemChange(item.id, "item", e.target.value)}
                          placeholder="Description of the item"
                          className="min-h-[40px] border-none focus-visible:ring-0 resize-none"
                        />
                      </td>
                      <td className="p-1">
                        <Textarea
                      ...
                      <td className="p-1">
                        <Input
                          ...
                          className="h-10 border-none focus-visible:ring-0 text-center"
                        />
                      </td>
                      <td className="p-1">
                        <Input
            ...
            {saveMessage && (
              <div className={`mt-4 p-3 rounded-md text-sm font-medium ${
                saveMessage.startsWith("✅") ? "bg-green-100 text-green-700" : saveMessage.startsWith("❌") ? "bg-red-100 text-red-700" : "bg-blue-100 text-blue-700"
              } no-print`}>
                {saveMessage}
              </div>
            )}
