...
      // Clear vinyl-specific fields if door_style changes away from 'vinyl'
      if (field === "door_style" && value !== "vinyl") {
        newState = { ...newState, door_manufacturer: "", door_name: "" };
      }
          
      return newState;
    });
...
      if (formData.door_style === "vinyl") {
        if (!formData.door_manufacturer?.trim()) errors.push("Door Manufacturer");
        if (!formData.door_name?.trim()) errors.push("Door Name");
      }
...
          {/* Customer Information - Blue Section (Layout Matching Image) */}
...
          {formType === "kitchen" && (
            <>
              {/* 1. Material Specifications - Green Section (Layout Matching Image) */}
...
                    {/* NEW: Door Manufacturer (Visible only for Vinyl) */}
...
                    {/* NEW: Door Name (Visible only for Vinyl) */}
                    {formData.door_style === "vinyl" && (
                      <div>
                        <label className="mb-1 block text-sm font-bold text-gray-700">Door Name</label>
                        <Input
                          placeholder="Enter door name (e.g., Kensington Matt White)"
                          className="w-full bg-white"
...
                    {/* Glazing Material - Only if Door Style is Glazed (Pushed to the end) */}
...
              {/* 2. Hardware Specifications - Purple Section (Layout Matching Image) */}
...
                      </select>
                      <select
                        className="w-full mt-2 rounded-md border border-gray-300 bg-white p-2"
...
              {/* 3. Worktop Specifications - Orange Section (Layout Matching Image) */}
...
                </div>
                
                <div className="mt-4 grid grid-cols-1 gap-y-4 md:grid-cols-2 md:gap-x-12">
                  <div>
                    <label className="mb-1 block text-sm font-bold text-gray-700">Under Wall Unit Lights</label>
                    <select
                      className="w-full rounded-md border border-gray-300 bg-white p-2"
...
              {/* 3. Bedroom Furniture - Orange Section (Layout Matching Image) */}
              <div className="mb-6 rounded-lg border-2 border-orange-200 bg-orange-50 p-6">
...
              {/* 4. Lighting - Yellow Section (Layout Matching Image) */}
              <div className="mb-6 rounded-lg border-2 border-yellow-200 bg-yellow-50 p-6">
